...
const log = (level: 'system' | 'warn' | 'error', message: string): void => {
...
function logWindowSlots(window: any): void {
    ...
    log('system', `[auth-debug] window opened — id=${window.id} type=${window.type} title="${title}"`);
    window.slots.forEach((item: any, index: number) => {
        if (!item) return;
        const custom = item.customName ? ` custom="${flattenChatComponent(item.customName)}"` : '';
        log('system', `[auth-debug]   slot ${index}: ${item.name} (${item.displayName})${custom}`);
    });
}
...
async function clickSlots(bot: Bot, slots: number[], delayMs: number): Promise<void> {
    for (const slot of slots) {
        try {
            ...
            log('error', `[auth] click on slot ${slot} failed: ${err?.message ?? err}`);
        }
        await sleep(delayMs);
    }
}
...
async function runGuiLogin(bot: Bot, cfg: AuthConfig): Promise<void> {
    ...
    if (slots.some(s => s === undefined)) {
        log('warn', '[auth] password has characters with no slot in config.auth.gui.slotMap — '
    ...
    log('system', '[auth] submitted password via GUI clicks');
}
...
async function runAnvilLogin(bot: Bot, cfg: AuthConfig): Promise<void> {
    try {
        ...
        log('system', '[auth] submitted password via anvil rename');
    } catch (err: any) {
        log('error', `[auth] anvil login failed: ${err?.message ?? err}`);
    }
}
...
async function runDialogLogin(bot: Bot, password: string): Promise<void> {
    ...
    const onResourcePack = (data: any) => {
        log('system', '[auth] dialog: acking resource pack (3→4→0)');
        try {
            ...
            log('error', `[auth] dialog: resource pack ack failed: ${err?.message ?? err}`);
        }
    };
    ...
    const onShowDialog = (data: any) => {
        ...
        log('system', `[auth] dialog: got login screen — clicking "${actionId}" with input "${inputKey}"`);
        if (!actionId) {
            log('error', '[auth] dialog: no actions in login screen — cannot submit');
            return;
        }
        try {
            ...
            log('system', '[auth] dialog: submitted password');
        } catch (err: any) {
            log('error', `[auth] dialog: submit failed: ${err?.message ?? err}`);
        }
    };
...
export function initAuth(bot: Bot, cfg: AuthConfig): void {
    if (!cfg.enabled) {
        log('system', '[AUTH] Auth disabled — skipping');
        return;
    }
    log('system', `[AUTH] Auth enabled — mode: ${cfg.mode}`);

    if (cfg.mode === 'dialog') {
        log('system', '[AUTH] Dialog login enabled — awaiting config-phase login screen');
    ...
    if (cfg.mode === 'command' || cfg.mode === 'both') {
        ...
        // Waiting for 'spawn' here would deadlock. 'login' fires immediately
        ...
        bot.once('login', () => {
            ...
            setTimeout(() => {
                ...
                try {
                    ...
                    log('system', '[auth] sent /login command');
                } catch (err: any) {
                    log('error', `[auth] failed to send /login: ${err?.message ?? err}`);
                }
            }, 1000);
        });
    }
    ...
    bot.on('windowOpen', async (window: any) => {
        ...
        log('system', `[auth] detected likely login window "${title}" — attempting GUI login`);
